...
"  1. exit command-line mode, execute one normal-mode command by entering insert
"     mode, feeding a <c-o> and waiting for |InsertEnter|, resume command-line
"     mode, and rebuild incsearch state (`incsearch_state_T is_state;` in
"     `getcmdline()` in ex_getln.c); or
...
" attempting to rebuild incsearch state sounds tricky and bug prone, so we opt
" for option 2. the expression register is evaluated under |textlock| (see
" `cmdline_paste()` in ex_getln.c for |c_CTRL-R| and `cmdline_handle_ctrl_bsl()`
" in ex_getln.c for |c_CTRL-\_e|), but |<cmd>| isn't, so that's what we run
" :normal with.

" here are the bits of Vim relevant to our makeshift input parsing, located in
" normal.c unless noted otherwise:
"   - `nv_cmds[]` in nv_cmds.h
"   - `normal_cmd()`, `normal_cmd_get_count()`, `normal_cmd_needs_more_chars()`,
...
cnoremap <plug>CCtrlOGo <cmd>echohl ModeMsg
      ...
      "\ have to call :redraw from the expression register instead of calling it
      "\ directly, otherwise scrolling using say <c-o><c-d> flickers but doesn't
      "\ scroll.
      \ <bar>call feedkeys("<c-r>=execute('redraw')[-1]\n", 'ni')

cnoremap <plug>CCtrlOOneShot <plug>CCtrlOGo<cr>
" using <bs> because the |line-continuation| adds a trailing space. also, note
" that <c-c> (and <esc> and <c-\><c-n> for that matter) actually goes to insert
" mode when the command-line was invoked from |i_CTRL-O|, and we want to
